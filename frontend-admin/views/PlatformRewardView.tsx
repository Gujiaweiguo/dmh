...
export default {
  ...
  setup() {
    ...
    const loadRewards = async () => {
      ...
      try {
        ...
        const { data } = await axios.get('/api/v1/platform/rewards', { params });
        if (data.code === 200) {
  ...
  template: `
    <div class="platform-reward-view">
      <div class="header">
        <h2>全局奖励管理</h2>
        <div class="actions">
        ...
        <table class="table">
          ...
          <tbody v-if="!loading">
            <tr v-for="reward in rewards" :key="reward.id">
              ...
              <td>{{ levelText(reward.level) }}</td>
              <td>{{ reward.rewardRate }}%</td>
