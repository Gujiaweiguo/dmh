...
func TestGetDistributorStatisticsHandler_Success(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodGet, fmt.Sprintf("/api/v1/distributors/%d/statistics", distributor.Id), nil)
	req.SetPathValue("id", fmt.Sprintf("%d", distributor.Id))
	resp := httptest.NewRecorder()
...
func TestTrackDistributorLinkHandler_Success(t *testing.T) {
	db := setupDistributorHandlerTestDB(t)
	...
	req := httptest.NewRequest(http.MethodPost, fmt.Sprintf("/api/v1/distributors/%s/track", distributor.Code), nil)
	resp := httptest.NewRecorder()
...
func TestGetDistributorSubordinatesHandler_Success(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodGet, fmt.Sprintf("/api/v1/distributors/%d/subordinates?page=1&pageSize=10", distributor.Id), nil)
	req.SetPathValue("id", fmt.Sprintf("%d", distributor.Id))
	resp := httptest.NewRecorder()
